# Chapter ﻿19

*Campbell Biology, AP Edition, 12e*

---

<!-- Page 458 -->
![Micrograph showing human immune cell releasing scores of human immunodeficiency viruses.](../images/img_p458_0.jpeg)


# **Chapter ﻿19**

# Viruses


#### Figure 19.1


A human immune cell (blue) infected by human immunodeficiency virus (HIV) is releasing more HIV viruses (gold dots), which will go on to infect other cells. Left untreated, HIV destroys vital immune system cells, causing AIDS. This disease has killed about  35 million people worldwide.


![A diagram of virus replication. How does a virus makes more virus?](../images/img_p458_1.jpeg)


A virus consists only of nucleic acid, proteins, and sometimes a membranous envelope. After infecting a host cell, it uses the host cell’s molecules to make new viruses, as shown in this simplified diagram. A generic virus containing a viral genome, D N A or R N A, enters a host cells and releases the genome. The viral genome is replicated by host enzymes. Copies of the genome are transcribed by host enzymes. Viral m R N A is translated by host ribosomes. Viral proteins are self assembled with the viral genome. New viruses are released from the cell. With some viruses, the host cell bursts when the new viruses are released. With some viruses, the host cell remains alive after the new viruses are released.


## Key Concepts

- **19.1****A virus consists of a nucleic acid surrounded by a protein coat**
- **19.2****Viruses replicate only in host cells**
- **19.3****Viruses and prions are formidable pathogens in animals and plants**


![Advanced placement edition](../images/img_p458_2.png)


Viral replication is unique in that it is dependent on the cellular machinery of its host **(BIG IDEA 3)**. Viruses can also introduce variation into the genetic material of a host, which can lead to rapid evolutionary change **(BIG IDEA 1)**. Disruption to ecosystems can expose plants and animals to different viruses (**BIG IDEA 4)**.


## Study Tip

**Make flowcharts:** Make a simple flowchart for each viral replicative cycle in this chapter. Below is the beginning of an example for the lysogenic cycle. Note the similarities and differences between the replicative cycles.


![The lysogenic cycle. Virus attaches to bacterial cell and injects genome. Viral genome is integrated into the bacterial chromosome.](../images/img_p458_3.jpeg)


## Get Ready for This Chapter

In this chapter, you will learn about how viruses were discovered, their structure and means of replication, and their impact in terms of disease. Prions as infectious proteins are also described. Before beginning this chapter, you should be able to do the following:

- Distinguish between the structures of a bacterial cell and a eukaryotic cell (see Concept 6.2, Figure 6.5, and Figure 6.8).
- Explain how two proteins bind to each other (see Concept 5.4).
- Describe the process of gene expression (see Concept 17. 2, Concept 17.4, and Figure 17.4).

The following questions provide a quick check of your basic knowledge in these areas.


1. Which of the following clues would tell you if a cell is prokaryotic or eukaryotic?


            the presence or absence of ribosomes


That's incorrect. Try again.


**Incorrect.** Review the following material: All cells are bounded by a selective barrier called the plasma membrane, and all cells have ribosomes, tiny complexes that make proteins according to the instructions from DNA. Eukaryotic cells, however, are generally much larger than prokaryotic cells and have internal membranes, which are not present in most prokaryotic cells. In a eukaryotic cell, most of the DNA is in an organelle called the nucleus, whereas in prokaryotic cells, the DNA is concentrated in a region that is not membrane-enclosed, called the nucleoid.


            whether or not the cell carries out cellular metabolism


That's incorrect. Try again.


**Incorrect.** Review the following material: All cells are bounded by a selective barrier called the plasma membrane, and all cells have ribosomes, tiny complexes that make proteins according to the instructions from DNA. Eukaryotic cells, however, are generally much larger than prokaryotic cells and have internal membranes, which are not present in most prokaryotic cells. In a eukaryotic cell, most of the DNA is in an organelle called the nucleus, whereas in prokaryotic cells, the DNA is concentrated in a region that is not membrane-enclosed, called the nucleoid.


            whether or not the cell is partitioned by internal membranes


That's correct.


**Correct!** If you had trouble with this question, review the following material: All cells are bounded by a selective barrier called the plasma membrane, and all cells have ribosomes, tiny complexes that make proteins according to the instructions from DNA. Eukaryotic cells, however, are generally much larger than prokaryotic cells and have internal membranes, which are not present in most prokaryotic cells. In a eukaryotic cell, most of the DNA is in an organelle called the nucleus, whereas in prokaryotic cells, the DNA is concentrated in a region that is not membrane-enclosed, called the nucleoid.


            the presence or absence of a rigid cell wall


That's incorrect. Try again.


**Incorrect.** Review the following material: All cells are bounded by a selective barrier called the plasma membrane, and all cells have ribosomes, tiny complexes that make proteins according to the instructions from DNA. Eukaryotic cells, however, are generally much larger than prokaryotic cells and have internal membranes, which are not present in most prokaryotic cells. In a eukaryotic cell, most of the DNA is in an organelle called the nucleus, whereas in prokaryotic cells, the DNA is concentrated in a region that is not membrane-enclosed, called the nucleoid.


2﻿. What determines whether two proteins will bind to each other?


            They will bind to each other if they are located in the same region of the cell.


That's incorrect. Try again.


**Incorrect.** Review the following material: A protein’s specific structure determines how it works, and in almost every case, the function of a protein depends on its ability to recognize and bind to some other molecule. This fit is very specific, something like the fit of two hands in a handshake, and depends on both the shape and the overall chemical properties of the regions where the proteins might bind to each other.


            They will bind to each other if one protein has an overall positive charge and one protein has an overall negative charge.


That's incorrect. Try again.


**Incorrect.** Review the following material: A protein’s specific structure determines how it works, and in almost every case, the function of a protein depends on its ability to recognize and bind to some other molecule. This fit is very specific, something like the fit of two hands in a handshake, and depends on both the shape and the overall chemical properties of the regions where the proteins might bind to each other.


            They will bind to each other if they have complementary shapes and chemical properties.


That's correct.


**Correct!** If you had trouble with this question, review the following material: A protein’s specific structure determines how it works, and in almost every case, the function of a protein depends on its ability to recognize and bind to some other molecule. This fit is very specific, something like the fit of two hands in a handshake, and depends on both the shape and the overall chemical properties of the regions where the proteins might bind to each other.


            They will bind to each other if they are identical or similar proteins.


That's incorrect. Try again.


**Incorrect.** Review the following material: A protein’s specific structure determines how it works, and in almost every case, the function of a protein depends on its ability to recognize and bind to some other molecule. This fit is very specific, something like the fit of two hands in a handshake, and depends on both the shape and the overall chemical properties of the regions where the proteins might bind to each other.


3﻿. Which of the following events occurs during the process of transcription?


            An RNA is synthesized.


That's correct.


**Correct!** If you had trouble with this question, review the following material: Transcription is the synthesis of RNA using the information in DNA. Translation is the synthesis of a polypeptide using the information in mRNA. The sites of translation are ribosomes, molecular complexes that facilitate the orderly linking of amino acids into polypeptide chains.


            An mRNA attaches to a ribosome.


That's incorrect. Try again.


**Incorrect.** Review the following material: Transcription is the synthesis of RNA using the information in DNA. Translation is the synthesis of a polypeptide using the information in mRNA. The sites of translation are ribosomes, molecular complexes that facilitate the orderly linking of amino acids into polypeptide chains.


            DNA is replicated.


That's incorrect. Try again.


**Incorrect.** Review the following material: Transcription is the synthesis of RNA using the information in DNA. Translation is the synthesis of a polypeptide using the information in mRNA. The sites of translation are ribosomes, molecular complexes that facilitate the orderly linking of amino acids into polypeptide chains.


            A polypeptide is synthesized.


That's incorrect. Try again.


**Incorrect.** Review the following material: Transcription is the synthesis of RNA using the information in DNA. Translation is the synthesis of a polypeptide using the information in mRNA. The sites of translation are ribosomes, molecular complexes that facilitate the orderly linking of amino acids into polypeptide chains.


## Learning Objectives

After studying this chapter, you should be able to do the following:

- 19.1 Describe a virus.
- 19.2 Explain how viruses replicate and evolve.
- 19.3 Use examples to illustrate the effects of animal and plant viruses, as well as prions, on their hosts.

---

<!-- Page 459 -->
