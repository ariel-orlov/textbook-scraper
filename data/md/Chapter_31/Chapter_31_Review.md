...
- In fungi, the sexual life cycle involves cytoplasmic fusion (**plasmogamy**) and nuclear fusion (**karyogamy**), with an intervening **heterokaryotic** stage in which cells have haploid nuclei from two parents. The diploid cells resulting from karyogamy are short-lived and undergo meiosis, producing genetically diverse haploid **spores******.
...
| --- | --- |
| Cryptomycota (cryptomycetes) | Parasites with flagellated spores. An image of a fungus invaded by parasitic spores of Cryptomycetes. |
| Microsporidia (microsporidians) | Parasitic cells that form resistant spores. A parasitic cell of Microsporidia, which forms resistant, elliptical spores. |
| Chytridiomycota (chytrids) | Flagellated spores. An image of a flagellated chytrid. |
| Zoopagomycota (zoopagomycetes) | Resistant zygosporangium as sexual stage. An image illustrating the zoosporangium of Zoopagomycetes, in a sexual stage. |
| Mucuromycota (mucuromycetes) | Include fungi that form arbuscular mycorrhizae with plants. An image illustrating a fungus forming arbuscular mycorrhizae with plants, belonging to Mucoromycetes. |
| Ascomycota (ascomycetes) | Sexual spores (ascospores) borne internally in sacs called asci; vast numbers of asexual spores (conidia) produced). An image illustrating the release of sexual spores (ascospores) from the asci through the ascocarp of an ascomycete, bread mold (Neurospora). |
| Basidiomycota (basidiomycetes) | Elaborate fruiting body (basidiocarp) containing many basidia that produce sexual spores (basidiospores). A part of the life cycle of a basidiomycete showing the fruiting body basidiocarp releasing basidiospores from a basidium. |


**DRAW IT** *Draw a phylogenetic tree of the major groups of fungi.*
