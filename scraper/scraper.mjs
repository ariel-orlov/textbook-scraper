...
async function fetchVerificationCode(maxWaitSecs = 120) {
  ...
  while (Date.now() - startTime < maxWaitSecs * 1000) {
    try {
      ...
      // Search for recent emails from Pearson
      ...
      if (messages.length > 0) {
        ...
        // Parse the email properly to get decoded text content
        ...
        // Look for the code in the plain text body (no CSS hex colors here)
        ...
        const patterns = [
          ...
          /Your code:\s*(\d{6})/i,
          /^\s*(\d{6})\s*$/m, // standalone 6-digit number on its own line
        ];
        ...
        // Fallback: find any 6-digit number in the text body,
        // filtering out common CSS-like values
    ...
    console.log("  No code yet, waiting 5 seconds...");
    await new Promise((r) => setTimeout(r, 5000));
  }

  console.log("  ⚠️ Timed out waiting for verification code.");
  return null;
}
...
async function login(page) {
  ...
  if (pageText.includes("Multifactor") || pageText.includes("verification code")) {
    ...
    const sendResult = await clickElementByText(page, "Send verification code");
    console.log(`  Clicked send: ${sendResult || "not found"}`);
    await new Promise((r) => setTimeout(r, 3000));
    ...
    if (code) {
      // Find the code input field using an element handle so we can use Puppeteer's .type()
      await new Promise((r) => setTimeout(r, 2000));

      // Get a handle to the code input
      const codeInput = await page.evaluateHandle(() => {
        ...
        for (const input of inputs) {
          if (input.type === "text" || input.type === "number" || input.type === "tel" ||
              ...
              input.getAttribute("placeholder")?.toLowerCase().includes("code")) {
            return input;
          }
        }
        // Fallback: find any visible text-like input that isn't email/password
        for (const input of inputs) {
      ...
      if (inputElement) {
        // Clear existing value and type the code using Puppeteer's native method
        await inputElement.click({ clickCount: 3 });
        await page.keyboard.press("Backspace");
        await inputElement.type(code, { delay: 50 });
        console.log(`  Code "${code}" typed into field.`);
        await screenshot(page, "mfa_code_entered");
        ...
        await page.waitForNavigation({ waitUntil: "networkidle2", timeout: 30000 }).catch(() => {});
        await new Promise((r) => setTimeout(r, 5000));
        await screenshot(page, "mfa_complete");
        console.log("✅ MFA completed automatically!");
      } else {
        console.log("  Could not find code input. Falling back to manual...");
        const mfaStart = Date.now();
        while (Date.now() - mfaStart < 300000) {
          await new Promise((r) => setTimeout(r, 2000));
          const t = await page.evaluate(() => document.body.innerText);
          if (!t.includes("Multifactor") && !t.includes("Check your email") && !t.includes("verification code")) break;
        }
      }
    } else {
      console.log("  Could not fetch code from Gmail. Complete MFA manually.");
      const mfaStart = Date.now();
      while (Date.now() - mfaStart < 300000) {
        await new Promise((r) => setTimeout(r, 2000));
        const t = await page.evaluate(() => document.body.innerText);
